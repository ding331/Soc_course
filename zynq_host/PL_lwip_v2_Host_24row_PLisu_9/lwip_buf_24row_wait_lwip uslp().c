...
//	u32 *lwip_buf_32;
	 ...
	 */
		...
		// for (Index = 0; Index < Tries; Index ++) {
			...
			//tidy
			// xil_printf("DMA done");
		for (int rows_24 = 0; rows_24 < 24; n++) {   //ini row
			// for (int n = rows_24*(720*3+100); n < 4+ rows_24*(720*3+100); n++) {   //ini row
			// 	lwip_buf[n] = 0;
			// }
			// for (int n = rows_24*(4+ 720*3)*2164; n < rows_24*(4+ 720*3)*2264; n++) {   //ini row 4+3*720
			// 	lwip_buf[n] = 0;
			// }
		}
		for (rows_24 = 0; rows_24 < 24; rows_24++) {	//24 row 1 buf[]
            RxBufferPtr_32 = RX_BUFFER_BASE + (u32)(720*(24*row));   //
		// 	// // xil_printf("&RxBufferPtr_32 sh  %x \r\n", &RxBufferPtr_32);
		// 	xil_printf("*RxBufferPtr_32 sh  %x \r\n", *RxBufferPtr_32);

			for (int j = 720*rows_24; j < 720*(rows_24+1); j++) {
				// for (int i = 0; i < 3; i++) {
				lwip_buf[4 + 3 * j] =     (u8)((RxBufferPtr_32[j] >> 24) & 0xF0); // 31 downto 28 bits with 4 bits 0
				lwip_buf[4 + 3 * j + 1] = (u8)((RxBufferPtr_32[j] >> 20) & 0x0F0); // 27 downto 24 bits with 4 bits 0
				lwip_buf[4 + 3 * j + 2] = (u8)((RxBufferPtr_32[j] >> 16) & 0x00F0); // 23 downto 20 bits with 4 bits 0
				// }
		...
		// }	//DDR send 2 hos
			...
			if (Error==0) {
				...
				usleep(350000);	//pac lost per 20 time
			}
