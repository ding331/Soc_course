...
//	u32 *lwip_buf_32;
	...
	// SD_Transfer_read("road720.bin", (u32)(value + 720), 720);
	// file_pointer += 720;
	//1st back frame
	while (page < 3) {
		// row = 0;
		...
		*/
			...
			// xil_printf("...Xil_DCacheFlushRlwip_bufange");
				Xil_DCacheFlushRange((UINTPTR)lwip_buf, 721*4);	//(4 + 720*3 + 100)/4);	//*
			...
			// }	//DDR send 2 hos
				...
				if (Error==0) {
					...
					if (SendResults == 1){
						SendResults = 0;						//***
						psnd = pbuf_alloc(PBUF_TRANSPORT, (4+720 + 100)*sizeof(u8), PBUF_REF);
					...
					usleep(250000);	//pac lost per 20 time
				}
