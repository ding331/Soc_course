...
//	u32 *lwip_buf_32;
	...
	// init_platform();
	 ...
	 IP4_ADDR(&ipaddr,  192, 168,   1, 77);		//192, 168, 178, 148
	 IP4_ADDR(&netmask, 255, 255, 255,  0);//
	 IP4_ADDR(&gw,      192, 168,   1,  1);		//192, 168, 178, 254

	//  IP4_ADDR(&ipaddress,  192, 168, 1, 77); /* iperf server address */
...
//				tcp_send_init();	//
				...
				if(tcp_client_connected)
				{
					...
					if (tcp_sndbuf(tpcb) > lwip_len)
					{
						xil_printf("tc2:");

					/*transmit received data through TCP*/

						err = tcp_write(tpcb, lwip_buf_32, lwip_len, copy);
						if (err != ERR_OK) {
