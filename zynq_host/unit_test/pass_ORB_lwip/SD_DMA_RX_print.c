...
//	u32 *lwip_buf_32;
	 ...
	 */
		...
		for (row = 0; row < 480; row++) {   //ini row
			...
			for (int j = 0; j < 720 ; j++) {
				if (btnMode == 1) {	//ORB ...444					//1 u32 = 1 pixel
					...
					{	// erosion, bypass DMA_TX[7:0], bypass DMA_TX[31:24]
				...
				if (RxBufferPtr_32[j+720*row] != 0 && row < 183)
				{
					xil_printf("x, y pos:  %d , %d \r", j , row);
					xil_printf("RxBufferPtr_32[] != 0 :  %d \r\n", RxBufferPtr_32[j+720*row]);
				}
			...
			// 		lwip_buf[4+2160 *(row% 24) + j * 5]     = (u8)((RxBufferPtr_32[j+720*row] >> 12) & 0xFF); // 取最低20bits之後的前8bits
			// 		lwip_buf[4+2160 *(row% 24) + j * 5 + 1] = (u8)((RxBufferPtr_32[j+720*row] >> 4) & 0xFF); // 取最低20bits之後的中間8bits

			// // 提取Rx4+2160 + BufferPtr[j*2]和RxBufferPtr[j*2+1]的特定位數到lwip_buf[j*5+2]到lwip_buf[j*5+4]
			// 		lwip_buf[4+2160 * (row% 24) + j * 5 + 2] = (u8)(((RxBufferPtr_32[j+720*row] << 4) & 0xF0) | ((RxBufferPtr_32[j+720*row + 1] >> 16) & 0x0F)); // RxBufferPtr_32[j*2]取最低20bits之後的最後4bits | (RxBufferPtr_32[j*2+1] & 0xFFFFF)的前4bits
			// 		lwip_buf[4+2160 * (row% 24) + j * 5 + 3] = (u8)((RxBufferPtr_32[j+720*row + 1] >> 8) & 0xFF); // 取RxBufferPtr[j*2+1]最低20bits之後的中間8bits
			// 		lwip_buf[4+2160 * (row% 24) + j * 5 + 4] = (u8)(RxBufferPtr_32[j+720*row + 1] & 0xFF); // 取RxBufferPtr[j*2+1]最低20bits之後的後8bits
		...
		// }	//DDR send 2 hos
			...
			// 	// detect_cnt = 0;
//			 }
			// 	usleep(120000);	//pac lost per 20 time
		}
			xil_printf("\n page lwip_cnt = %d \r\n",lwip_cnt);
			lwip_cnt = 0;
