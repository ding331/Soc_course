		...
		for (row = 0; row < 480; row++) {   //ini row
			...
			for (int j = 0; j < 720 ; j++) {
				...
				if ((RxBufferPtr_32[j] << 12 & 0xFFFFF) != 0) {
					...
					lwip_buf[4+2160 *row + j * 5]     = (u8)((RxBufferPtr_32[j] >> 12) & 0xFF); // 取最低20bits之後的前8bits
					lwip_buf[4+2160 *row + j * 5 + 1] = (u8)((RxBufferPtr_32[j] >> 4) & 0xFF); // 取最低20bits之後的中間8bits

			// 提取Rx4+2160 + BufferPtr[j*2]和RxBufferPtr[j*2+1]的特定位數到lwip_buf[j*5+2]到lwip_buf[j*5+4]
					lwip_buf[4+2160 * row + j * 5 + 2] = (u8)(((RxBufferPtr_32[j] << 4) & 0xF0) | ((RxBufferPtr_32[j + 1] >> 16) & 0x0F)); // RxBufferPtr_32[j*2]取最低20bits之後的最後4bits | (RxBufferPtr_32[j*2+1] & 0xFFFFF)的前4bits
					lwip_buf[4+2160 * row + j * 5 + 3] = (u8)((RxBufferPtr_32[j + 1] >> 8) & 0xFF); // 取RxBufferPtr[j*2+1]最低20bits之後的中間8bits
					lwip_buf[4+2160 * row + j * 5 + 4] = (u8)(RxBufferPtr_32[j + 1] & 0xFF); // 取RxBufferPtr[j*2+1]最低20bits之後的後8bits
