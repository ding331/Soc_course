...
video_start = 0
video_end = 200
vidcap = cv2.VideoCapture('rec_move.mp4')
...
def getFrame_BIN8bits(sec, bin_file):
    ...
    if hasFrames:
        resized_image = resize_image(image, 720, 480)
        cropped_image = crop_center(resized_image, 720, 480)
...
bin_file = '/rec_opt_for_test/rec_opt_for_test.bin'
rot_bin_file = '/rec_opt_for_test/rec_opt_for_test2.bin'
