...
#define RX_BUFFER_BASE		(MEM_BASE_ADDR + 0x00300000)    //0x00300000
#define RX_BUFFER_HIGH		(MEM_BASE_ADDR + 0x004FFFFF)

#define BUFFER_BASE		(MEM_BASE_ADDR + 0x00451810)   //0x01751804
...
#endif
{
	 ...
	 */
        ...
        for (int n = 4+3*720; n < 4+3*720 +100; n++) {   //ini row
            lwip_buf[n] = 0;
        }
		// xil_printf("lwip_buf[0] base addr place %x \r\n",lwip_buf[0]);
		// xil_printf("lwip_buf[4+3*720 ] 0x %x \r\n",&lwip_buf[4+3*720 ]);    //0x1752078
        ...
        for (row = 0; row < 480; row++) {	//
            RxBufferPtr_32 = RX_BUFFER_BASE + (u32)(720*(row));   //
                ...
                Xil_DCacheFlushRange((UINTPTR)lwip_buf, 4 + 720*3 + 100);	//別[0] mem 測的
             // xil_printf("Cache..");
		...
		//last row
		xil_printf("&RxBufferPtr[] last addr %x \r\n",&RxBufferPtr[(MAX_PKT_LEN-720*4)]);
		// xil_printf("RxBufferPtr[] last addr %x \r\n",RxBufferPtr[(MAX_PKT_LEN-720*4)]);
		xil_printf("RxBufferPtr_32 0x %x \r\n",RxBufferPtr_32);
		xil_printf("&lwip_buf[0] base addr place %x \r\n",&lwip_buf[0]);
	}
	lwip_buf_32 = (u32*)BUFFER_BASE;
	
        for (u32 n = 0; n < 4; n++) {
            xil_printf("lwip_buf[%x] 0x %x \r\n", n,  lwip_buf_32[n]);
        }
                // xil_printf("tot row  = %x \r\n", (u32*)lwip_buf[0]);
		detect_cnt = 0;
		page = page + 1;
