        ...
        if lwip_cnt != last_lwip_cnt:
            ...
            for pac_index in range(24):  # 0~4, 724~728,        ***
                ...
                if cur_line == IM_Y - 1:
                    print("page : ", page)
                    cv2.imwrite(
                        f'static/img/SD_image_{page}.bmp', image_array)
                    if page == 1:
                        # handle_image(image_array)
                        prev_img = read_image(page)

                # if page % 5 == 0 and page > 0:
                    # for page in range(2, page):
                    else:   #page >= 2
                        # for page in range(page-1, page):  # refresh CSV, vido, etc
                        ...
                        cv2.imwrite(f'optical_flow_output_{page}.bmp', output_img)  # 另存為每10張的光流畫面
                        video_output.write(output_img)  # 寫入視頻
                        # handle_image(output_img)    #***
