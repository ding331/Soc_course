...
static u32 DMA_O_dest_str[imageSize];

//u32 *reorganized_DMA_O_dest_str = (u32 *) DMA_O_dest_str;
 u32 RX_buf[345600];	//volatile
...
//    }
	...
	XScuGic_SetPriorityTriggerType(&IntcInstance,XPAR_FABRIC_AXI_DMA_1_MM2S_INTROUT_INTR,0xA0,3);	//
	XScuGic_SetPriorityTriggerType(&IntcInstance,XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR,0xA0,3);	//
//	status = XScuGic_Connect(&IntcInstance,XPAR_FABRIC_AXI_DMA_1_MM2S_INTROUT_INTR,(Xil_InterruptHandler)imageProcISR,(XAxiDma *)&myDma2);	//
	if(status != XST_SUCCESS){
...
//			reorganized_DMA_O_dest_str = ((page & 1U) == 0U) ? value&(value +imageSize) : value;
				for (i = 0; i < 345600; i++) {
					DMA_O_dest_str[i] = (reorganized_value[i +imageSize ] & 0xFF) |
													((0x00) << 8) |
													((0x00) << 16) |
													((reorganized_value[i] & 0xFf) << 16);
			 ...
			 // Xil_DCacheFlushRange((u32)DMA_O_dest_str, 12382400);				//720*240

			 XAxiDma_SimpleTransfer(&myDma2, (u32)RX_buf[0], 345600, XAXIDMA_DEVICE_TO_DMA);
			XAxiDma_SimpleTransfer(&myDma, (u32)DMA_O_dest_str, 345600, XAXIDMA_DMA_TO_DEVICE);//typecasting in C/C++
			Xil_DCacheFlushRange((u32)RX_buf[0], 345600);
...
static void imageProcISR(XAxiDma *CallBackRef){	//(u32*)&RX_buf[720]
	...
	XScuGic_Disable(&IntcInstance,XPAR_FABRIC_AXI_DMA_1_MM2S_INTROUT_INTR);
	XScuGic_Disable(&IntcInstance,XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR);
...
//	if(j<720){															//j*720
		// Xil_DCacheFlushRange((u32)RX_buf[0], 2880);
		// XAxiDma_SimpleTransfer((XAxiDma *)CallBackRef,(u32)RX_buf[0],2880,XAXIDMA_DEVICE_TO_DMA);
		// }
