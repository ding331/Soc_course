...
static u32 DMA_O_dest_str[2*frameSize]; // Adjust the size to match the desired read length
...
int main() {
    ...
    while (page < 2) {
        ...
        for (u32 i = 0; i < frameSize; i++) {
			...
			//reorganized_DMA_O_dest_str[i] = (u32) i;	//test ILA mm2s
        }
	
        xil_printf("value %x\n\r", reorganized_DMA_O_dest_str[0]);

        Xil_DCacheFlushRange((u32)value, 2*frameSize);
        status = XAxiDma_SimpleTransfer(&myDma, (u32)reorganized_DMA_O_dest_str, 2*frameSize, XAXIDMA_DMA_TO_DEVICE);//typecasting in C/C++
        if (status != XST_SUCCESS)
...
// Rearrange the lower 8 bits of DMA_rx[0] to DMA_rx[4] and replace DMA_rx[0]
   for (int j = 0; j < 720/4; j++) {
     for (int i = 0; i < 4; i++) {
        DMA_rx[j]|= (DMA_rx[(4*j+i)] &0x000000FF) << (8 * i);
    }
     DMA_rx[j+1] = 0;
   }
	
	//&line,  DMA_rx[720/4], if (31 :11) != 0 then seriel matchPos to DMA_rx[5+181]... lw
	line = line +1 ;
	XScuGic_Enable(&IntcInstance,XPAR_FABRIC_AXI_INORB2_0_ORB_INTR_INTR);
}
